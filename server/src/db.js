...
const MIGRATIONS = [
  ...
  },
  {
    ...
    sql: `
      ...
      DROP TRIGGER IF EXISTS trg_kitchen_items_updated_at ON kitchen_items;
      CREATE TRIGGER trg_kitchen_items_updated_at
      ...
      CREATE INDEX IF NOT EXISTS idx_kitchen_orders_day_date ON kitchen_orders(day_date);

      CREATE TABLE IF NOT EXISTS kitchen_order_items (
        id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        order_id BIGINT NOT NULL REFERENCES kitchen_orders(id) ON DELETE CASCADE,
        item_id BIGINT NOT NULL REFERENCES kitchen_items(id),
...
async function seedDefaults() {
  ...
  const ki = await query('SELECT COUNT(*)::int AS c FROM kitchen_items')
  if ((ki.rows[0]?.c ?? 0) === 0) {
    const items = [
      { name: 'Pap', category: 'meal', priceCents: 2500 },
      { name: 'Steak', category: 'meal', priceCents: 4500 },
      { name: 'Hardbody Chicken', category: 'meal', priceCents: 4000 },
      { name: 'Bones', category: 'meal', priceCents: 3500 },
      { name: 'Coca Cola', category: 'drink', priceCents: 1500 },
      { name: 'Sprite', category: 'drink', priceCents: 1500 },
      { name: 'Fanta', category: 'drink', priceCents: 1500 },
      { name: 'Cream Soda', category: 'drink', priceCents: 1500 },
    ]
    for (let i = 0; i < items.length; i += 1) {
      const it = items[i]
      await query(
        'INSERT INTO kitchen_items (name, category, price_cents, sort_order, active) VALUES ($1, $2, $3, $4, 1) ON CONFLICT (name) DO NOTHING',
        [it.name, it.category, it.priceCents, i],
      )
    }
  }
}
